        ...
        <div class="sticky top-0 z-50 bg-ink text-surface">
            <div class="mx-auto flex max-w-4xl flex-col items-center justify-between gap-2 px-4 py-2 text-sm sm:flex-row sm:px-6">
                <p class="flex items-center gap-2">
                    ...
                    {{ $sample && $siteTemplate ? 'Contoh template '.$siteTemplate->name : 'Pratonton kad anda' }}
                </p>
                <div class="flex gap-2">
                    @if ($sample)
                        ...
                        <a href="{{ auth()->check() ? route('site.edit', ['template' => $siteTemplate?->slug]) : route('register') }}" class="rounded-full bg-surface px-4 py-1.5 text-xs font-semibold text-ink transition hover:opacity-90">Guna template ini</a>
                    @else
