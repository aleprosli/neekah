...
<details data-popover class="relative">
    <summary class="relative flex size-9 cursor-pointer list-none items-center justify-center rounded-full transition select-none hover:bg-surface-muted [&::-webkit-details-marker]:hidden" :aria-label="__('pages.bell.notifikasi')">
    ...
    <div class="absolute top-full right-0 z-20 mt-2 w-80 max-w-[calc(100vw-2rem)] overflow-hidden rounded-2xl border border-line bg-surface-raised text-sm shadow-xl shadow-brand-900/10">
        <div class="flex items-center justify-between border-b border-line px-4 py-3">
            <p class="font-semibold">{{ __('pages.bell.notifikasi_2') }}</p>
            @if ($unreadCount)
                <form method="POST" action="{{ route('notifications.read') }}">
                    @csrf
                    @method('PUT')
                    <button type="submit" class="text-xs font-medium text-brand-600 hover:underline">{{ __('pages.bell.tandakan_dibaca') }}</button>
                </form>
            @endif
        </div>
        ...
        @empty
            <p class="px-4 py-8 text-center text-sm text-ink-muted">{{ __('pages.bell.tiada_notifikasi_baharu') }}</p>
        @endforelse

        <a href="{{ route('notifications.index') }}" class="block border-t border-line px-4 py-3 text-center text-xs font-medium text-brand-600 hover:bg-surface-muted">{{ __('pages.bell.lihat_semua') }}</a>
    </div>
</details>
