...
<template>
    ...
    <div class="mt-6 rounded-2xl border border-line bg-surface-raised p-5">
        <div class="flex flex-wrap items-center justify-between gap-x-3 text-sm">
            <span class="font-medium">{{ $t('budget.penggunaan_bajet') }}</span>
    ...
    <section class="mt-6 grid gap-4 lg:grid-cols-2">
        <div class="min-w-0 rounded-2xl border border-line bg-surface-raised p-5">
            <h2 class="font-semibold">{{ $t('budget.komitmen_mengikut_bulan') }}</h2>
            <p class="text-sm text-ink-muted">{{ $t('budget.jumlah_tempahan_yang_anda_buat') }}</p>
            <UiBarChart class="mt-4" :series="charts.committed" :empty="$t('budget.belum_ada_tempahan_untuk_dipaparkan')" />
        </div>

        <div class="min-w-0 rounded-2xl border border-line bg-surface-raised p-5">
            <h2 class="font-semibold">{{ $t('budget.bayaran_mengikut_bulan') }}</h2>
            <p class="text-sm text-ink-muted">{{ $t('budget.wang_yang_benar_benar_sudah') }}</p>
            <UiBarChart class="mt-4" :series="charts.paid" :empty="$t('budget.belum_ada_bayaran_direkod')" />
        </div>

        <div class="rounded-2xl border border-line bg-surface-raised p-5 lg:col-span-2">
            <h2 class="font-semibold">{{ $t('budget.perbelanjaan_mengikut_kategori') }}</h2>
            <UiDonutChart class="mt-4" :series="charts.categories" :empty="$t('budget.tempah_vendor_untuk_melihat_agihan')" />
        </div>
    </section>
    ...
    <form :action="action" method="POST" class="mt-6">
        ...
        <DataTable :rows="rows" :columns="COLUMNS" :csrf="csrf">
            ...
            <template #cell-vendors="{ row }">
                ...
                <a v-if="!row.bookings.length" :href="row.find_vendors_url" class="text-xs text-ink-muted hover:text-brand-700">{{ $t('budget.cari_vendor') }}</a>
            </template>
        </DataTable>

        <dl class="mt-3 flex flex-wrap justify-end gap-x-8 gap-y-1 rounded-2xl bg-surface-muted px-5 py-3 text-sm font-semibold">
            <div class="flex gap-2"><dt>{{ $t('budget.diagih') }}</dt><dd>{{ money(allocated) }}</dd></div>
            <div class="flex gap-2"><dt>{{ $t('budget.ditempah') }}</dt><dd>{{ totals.actual }}</dd></div>
        ...
        <div class="mt-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <label class="flex flex-col gap-1.5 sm:w-56">
                <span class="text-sm font-medium">{{ $t('budget.jumlah_bajet_majlis_rm') }}</span>
            ...
            <button type="submit" class="rounded-full bg-brand-600 px-8 py-3 text-sm font-semibold text-white transition hover:bg-brand-700">{{ $t('budget.simpan_bajet') }}</button>
        </div>
    </form>
</template>
