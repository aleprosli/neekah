...
<template>
    <div class="rounded-3xl border border-line bg-surface-raised p-6 text-center shadow-xl shadow-brand-900/5 sm:p-8">
        ...
        <template v-if="state === 'member'">
            ...
            <a :href="dashboardUrl" class="mt-3 inline-flex rounded-full bg-brand-600 px-6 py-3 text-sm font-semibold text-white transition hover:bg-brand-700">Buka majlis</a>
        </template>
        ...
        <template v-else>
            ...
            <a :href="browseUrl" class="mt-3 inline-block text-sm text-ink-muted underline underline-offset-4">Nanti dahulu</a>
        </template>
    </div>
